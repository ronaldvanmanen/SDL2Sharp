...
namespace SDL2Sharp
{
    public sealed unsafe class YuvTexture : Texture
    {
        ...
        public void Update(Span2D<byte> yPixels, Span2D<byte> uPixels, Span2D<byte> vPixels)
        {
            ThrowWhenDisposed();
            ...
            Update(null, yPlane, yPitch, uPlane, uPitch, vPlane, vPitch);
        }

        private void Update(SDL_Rect* rect, byte* yPlane, int yPitch, byte* uPlane, int uPitch, byte* vPlane, int vPitch)
        {
            Error.ThrowOnFailure(
                SDL.UpdateYUVTexture(this, rect, yPlane, yPitch, uPlane, uPitch, vPlane, vPitch)
            );
        }
    }
}
