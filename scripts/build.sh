...
function Test {
  ...
  if [[ -z "$properties" ]]; then
    dotnet test -c "$configuration" --no-build --no-restore -v "$verbosity" /bl:"$logFile" /err "$solution" -- RunConfiguration.DisableAppDomain=true
  else
...
if [[ -z "$solution" ]]; then
  solution="$RepoRoot/SDL2Sharp.sln"
fi
